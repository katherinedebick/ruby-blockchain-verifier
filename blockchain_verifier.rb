...
class BlockchainVerifier
  ...
  def verify(file)
    ...
    blockchain.each_line do |line|
      ...
      line_res = verify_block(current_block, @last_block)
  ...
  def verify_block(curr_block, last_block)
  ...
  def prev_hash_check(curr_block, last_block)
  ...
  def time_check(curr_block, last_block)
    unless last_block.nil?
      prev_arr = last_block.time_val.split('.')
      curr_arr = curr_block.time_val.split('.')
  ...
  def line_num_check(curr_block, last_block)
    return true if last_block.nil?
    return false unless curr_block.line_num.to_i == last_block.line_num.to_i + 1
    true
  end
